...
function unique(items: string[]) { return [...new Set(items)].slice(0, 8); }

function localAnalyze(story: string): Analysis {
  const lower = story.toLowerCase();
  const themes = Object.entries(themeMap).flatMap(([k, v]) => lower.includes(k) ? v : []);
  const safeThemes = unique(themes.length ? themes : ["identity shift", "emotional pattern", "longing for a truer future", "meaning-making"]);
  const blockers = unique([
    lower.includes("hate") || lower.includes("angry") ? "resentment becoming the author" : "old meaning shaping the ending",
    lower.includes("guilt") ? "guilt blocking self-permission" : "survival story repeating itself",
    lower.includes("parent") || lower.includes("family") ? "family pain defining identity" : "fear-based choice path",
    lower.includes("should") || lower.includes("expected") ? "living inside expectations" : "waiting for external rescue",
  ]);
  const primary = safeThemes[0] || "identity shift";
  return {
    summary: "EverAfter found a shared pattern inside the private story: the old wound is still trying to decide the future. In the real product, this pattern becomes the bridge into a community Reflection Room where others choose the same subject without seeing this user's private details.",
    pillars: {
      innovate: "The story is asking for a new perspective: pain can become a pattern to understand, not a permanent identity.",
      ...
      pinHome: "The AI moderator helps the room reach a specific realization that can be carried back into life.",
    },
    blockers,
    trajectory: [
    ...
    room: {
      ...
      prompt: "Stay with this one subject: what does this pattern protect, and what would help it move toward Home?",
      voices: [
